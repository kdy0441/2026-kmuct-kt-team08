...
def _stores(line: List[LatLng]) -> int:
    return sum(z.store_count for z in _near(_dataset()["store_zones"], line, AREA_M))
...
def edge_features(midpoints: Sequence[LatLng], departure: datetime) -> List[EdgeFeatures]:
    """선분 중점 목록 → 선분별 특징. 경로 단위 공식을 원형 반경으로 근사한다."""
    d = _dataset()
    ix = _indexes()
    hour = departure.hour
    night = hour in NIGHT_HOURS
    circle_100m = (2 * EDGE_RADIUS_M) / 100.0
    events = _active_events(departure)

    def sensor(key: str, _attr: str, p: LatLng) -> Optional[float]:
        return _sensor_at(key, hour, p, SENSOR_RADIUS_M)

    out: List[EdgeFeatures] = []
    for p in midpoints:
        # 조도 = max(가로등 밀도, 실측 lux)
        scores = []
        if ix["streetlights"] is not None:
            scores.append(min(100.0, len(ix["streetlights"].query(p, EDGE_RADIUS_M)) / circle_100m / LIGHTS_PER_100M_FULL * 100.0))
        lux = sensor("lighting", "illuminance_lux", p)
        if lux is not None:
            scores.append(_lux_score(lux))
        lighting = max(scores) if scores else MISSING_SCORE
        if scores and night and ix["store_zones"] is not None:
            if sum(d["store_zones"][i].store_count for i in ix["store_zones"].query(p, AREA_M)) >= 30:
                lighting = min(100.0, lighting + 10.0)
        # 혼잡
        event_near = any(haversine_m(p, e) <= AREA_M for e in events)
        if ix["crowd_grid"] is not None:
            cells = [d["crowd_grid"][i] for i in ix["crowd_grid"].query(p, CROWD_RADIUS_M) if d["crowd_grid"][i].time_slot == hour]
            crowd = _clamp(mean(c.crowd_score for c in cells) + (20.0 if event_near else 0.0)) if cells else MISSING_SCORE
        else:
            crowd = MISSING_SCORE
        # 조용함
        db = sensor("noise", "noise_level_db", p)
        quiet = _clamp(100.0 - (db - 40.0) * 2.5) if db is not None else (QUIET_DEFAULT if d["noise"] is not None else MISSING_SCORE)
        # 그늘
        if ix["shade"] is not None:
            spots = [d["shade"][i] for i in ix["shade"].query(p, EDGE_RADIUS_M)]
            shade = min(1.0, len(spots) / (2 * EDGE_RADIUS_M / SHADE_SPACING_M)) * mean(s.shade_score for s in spots) if spots else 0.0
        else:
            shade = MISSING_SCORE
        out.append(EdgeFeatures(lighting, crowd, quiet, shade, event_near))
    return out
