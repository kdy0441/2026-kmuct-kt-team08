...
class WalkGraph:
    ...
    def nearest_node(self, p: LatLng) -> int:
        # 1만 노드 선형 탐색 — 요청당 2번이라 충분히 빠르다
        return min(self.nodes, key=lambda n: haversine_m(p, self.nodes[n]))
...
def load_candidate_routes(
    ...
    start = g.nearest_node(geocode(request.start))
    end = g.nearest_node(geocode(request.end))
    if start == end:
    ...
    for i, (name, path) in enumerate(named):
        coords = [Coordinate(lat=g.nodes[n][0], lng=g.nodes[n][1]) for n in path]
        ...
        routes.append(
            CandidateRoute(
                ...
                hasStairs=steps > 0,
                coordinates=coords,
                metrics=data_service.compute_metrics(coords, distance, departure),
            )
        )
    if profile is None:
