...
def resolve_preference(request: RouteRequest) -> Tuple[PreferenceProfile, bool]:
    ...
    if _llm_parse_preference is not None:
        try:
            return _llm_parse_preference(request), False
...
def run_recommendation(request: RouteRequest) -> RouteRecommendationResponse:
    ...
    profile, preference_is_fallback = resolve_preference(request)
...
def recommend(request: RouteRequest) -> RouteRecommendationResponse:
